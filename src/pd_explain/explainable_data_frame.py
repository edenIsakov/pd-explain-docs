...
from explain_ed.Operations.Filter import Filter
from explain_ed.Operations.GroupBy import GroupBy
from explain_ed.Operations.Join import Join
from explain_ed import utils
from typing import (
    Hashable,
    Sequence,
    Union,
...
class ExpDataFrame(pd.DataFrame):
    ...
    def explain(self, schema: dict = None, attributes: List = None, top_k: int = 1,
                figs_in_row: int = 2, show_scores: bool = False):
        """
    ...
    def calc_subtab(self, use_rules: bool = True, subtub_config=None):
        """
        Calculate subtab for this dataframe

        :param use_rules: Whether to use rules or not
        :param subtub_config: dict for sub tab config, for example:
               subtub_config = {'NUM_COLS' : 20, # default 10
                                'NUM_ROWS': 100, # default 10
                                'prefix': 'example', # default os.getcwd()
                                'name_dataset': 'my_new_dataset', # default 'users_dataset'
                                'nulls_ratio': 1, # default 0
                                'MODEL_UUID': '1-1-1' # default {uuid.uuid4().hex}_{name_dataset}_binned_data_cell_to_vec,
                                'CONF': {}, # default  {'add_attr': 'True', 'add_columns': 'True',
                                                        'vector_size': 50, 'row_window_size': 10,
                                                        'col_window_size': 100, 'min_count': 2}}
        """
        if subtub_config is None:
            subtub_config = {}
        if not self.subtab:
            from pd_explain.utils import pandas_read, explain_read
            pd.io.parsers.readers._read = pandas_read
            self.subtab = SubTub(self, use_rules, subtub_config)
            pd.io.parsers.readers._read = explain_read

    def display(self, use_rules: bool = True, sub_config=None):
        """
        display dataframe subtab
        :param use_rules: Whether to use rules or not
        :param sub_config: dict for sub tab config, for example:
               sub_config = {'NUM_COLS' : 20, # default 10
                                'NUM_ROWS': 100, # default 10
                                'prefix': 'example', # default os.getcwd()
                                'name_dataset': 'my_new_dataset', # default 'users_dataset'
                                'nulls_ratio': 1, # default 0
        ...
        if not self.subtab:
            self.calc_subtab(use_rules, sub_config)
